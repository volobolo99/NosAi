...
def load_verified_templates(manifest: str|Path, minimum_confidence: float=.78) -> dict[str, list[Path]]:
    ...
    for item in data.get("items",[]):
        if item.get("status") not in {"verified", "production"}: continue
        if float(item.get("confidence",0.0)) < minimum_confidence: continue
        kind=str(item.get("kind","")); path=Path(item["path"])
        if kind and path.exists(): out.setdefault(kind,[]).append(path)
    return out

class VisualEntityDetector:
    """Detect player/NPC/mob candidates with optional OpenCV templates."""
    def __init__(self, templates: dict[str,str|Path|list[str|Path]]|None=None, threshold: float=.78, scales: tuple[float,...]=(1.0,)):
        self.templates={}
        for kind, value in (templates or {}).items():
            values=value if isinstance(value,(list,tuple)) else [value]
            self.templates[kind]=[Path(v) for v in values]
        self.threshold=threshold; self.scales=scales

    @classmethod
    def from_manifest(cls, manifest: str|Path, threshold: float=.78):
        return cls(load_verified_templates(manifest, minimum_confidence=threshold), threshold=threshold)
        ...
        for kind, paths in self.templates.items():
            for path in paths:
                ...
                for scale in self.scales:
                    ...
                    if scale != 1.0:
                        tw=max(2,int(tw*scale)); th=max(2,int(th*scale))
                        scaled=cv2.resize(template,(tw,th),interpolation=cv2.INTER_AREA if scale<1 else cv2.INTER_LINEAR)
                    else: scaled=template
                    if scaled.shape[0] > image.shape[0] or scaled.shape[1] > image.shape[1]: continue
                    result=cv2.matchTemplate(image,scaled,cv2.TM_CCOEFF_NORMED)
                    _,score,_,loc=cv2.minMaxLoc(result)
                    if score>=self.threshold:
                        out.append(Detection(kind,loc[0],loc[1],scaled.shape[1],scaled.shape[0],float(score),source="verified_template"))
        return tuple(out)
