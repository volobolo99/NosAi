...
if nn is not None:
    ...
    class _DynamicsNet(nn.Module):
        ...
        def __init__(self, state_dim: int, action_dim: int, latent_dim: int, hidden: int = 64) -> None:
            super().__init__()
        ...
        def forward(self, z: torch.Tensor, a: torch.Tensor) -> tuple:
            ...
            return next_latent, self.reward(h).squeeze(-1), self.done(h).squeeze(-1), self.state_head(next_latent)

else:

    class _DynamicsNet:  # pragma: no cover
...
class LatentWorldModel:
    ...
    def __init__(self, state_dim: int | None = None, action_dim: int = 3, latent_dim: int = 8,
                 hidden: int = 64, seed: int = 42, lr: float = 1e-3, device: str | None = None,
                 train_device: str | None = None, gpu_min_samples: int | None = None) -> None:
        if torch is None or nn is None:
            raise RuntimeError("PyTorch is required for the learnable LatentWorldModel")
        self.state_dim, self.action_dim, self.latent_dim = state_dim, action_dim, latent_dim
        self.hidden, self.seed, self.lr = hidden, seed, lr
        profile = detect_hardware()
        requested = device or profile.online_device
        self.device = torch.device(profile.online_device if requested == "auto" else requested)
        if self.device.type == "cuda" and not torch.cuda.is_available():
            raise RuntimeError("CUDA device requested but CUDA is unavailable")
        requested_train = train_device or profile.training_device
        requested_train = profile.training_device if requested_train == "auto" else requested_train
        ...
        self.gpu_min_samples = profile.gpu_training_min_samples if gpu_min_samples is None else max(1, int(gpu_min_samples))
        self._net = self._optimizer = None
    ...
    def _ensure(self, state_dim: int) -> None:
        ...
        for module in (self._encoder, self._net, self._decoder):
            module.to(self.device)
        self._reset_optimizer()

    def _reset_optimizer(self) -> None:
        self._optimizer = torch.optim.Adam(
            list(self._encoder.parameters()) + list(self._net.parameters()) + list(self._decoder.parameters()), lr=self.lr
        )
    ...
    def _action_vec(self, action: Action) -> list[float]:
        ...
        return (vals + [0.0] * max(0, self.action_dim - len(vals)))[:self.action_dim]
    ...
    def predict(self, state: State, action: Action) -> Prediction:
        ...
        next_state = State(tuple(float(v) for v in next_state_vec[0].tolist()), state.timestamp + 1, state.scenario_id, state.metadata)
    ...
    def train(self, transitions: Iterable, epochs: int = 25, batch_size: int = 32) -> dict:
        ...
        for _ in range(epochs):
            ...
            for start in range(0, n, batch_size):
                ...
                _, reward_pred, done_pred, next_state_pred = self._net(z, a[idx])
                loss = (((next_state_pred - y[idx]) ** 2).mean()
