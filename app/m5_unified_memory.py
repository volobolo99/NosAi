"""Unified memory abstraction over episodic, semantic, strategic and graph memory."""
...
class MemoryRecord:
    """Single memory record across all memory types."""
...
class UnifiedMemory:
    ...
    def __init__(
        ...
        reliability: MemoryReliability | None = None,
    ) -> None:
    ...
    def records(self) -> list[MemoryRecord]:
        """Get all memory records across all stores."""
        ...
        for x in store.observations.values():
            rows.append(
                MemoryRecord(
                    x.id,
                    "episodic",
                    f"{x.event_type} {x.payload}",
                    x.confidence,
                    (x.id,),
                    {"session_id": x.session_id, "timestamp": x.timestamp},
                )
            )
        for x in store.facts.values():
            rows.append(
                MemoryRecord(
                    x.id,
                    "semantic",
                    f"{x.subject} {x.predicate} {x.object}",
                    x.confidence,
                    tuple(x.source_refs),
                    {
                        "subject": x.subject,
        ...
        for x in store.inferences.values():
            rows.append(
                MemoryRecord(
                    ...
                    f"{x.subject} {x.predicate} {x.object}",
                    x.confidence,
                    tuple(x.supporting_observations),
                    {
                        "status": x.status,
                        "subject": x.subject,
                        "predicate": x.predicate,
        ...
        for i, x in enumerate(store.strategy_experiences):
            ...
            rows.append(
                MemoryRecord(
                    rid,
                    "strategy",
                    f"{x.goal_type} {x.strategy_id}",
                    1.0 if x.success else 0.5,
                    (),
                    {
    ...
    def retrieve(
        ...
        *,
        ...
        # Expand explicit entities through the lightweight graph.
        # The expansion is intentionally bounded so retrieval cost remains predictable.
        ...
        for record in self.records():
            if record.confidence < min_confidence or (
                allowed and record.kind not in allowed
            ):
                continue
            ...
            if timestamp is not None and query_tokens:
                try:
                    age_days = max(
                        0.0,
                        (datetime.now(timezone.utc) - timestamp).total_seconds() / 86400.0,
                    )
        ...
        hits.sort(
            key=lambda h: (h.score, h.record.confidence, h.record.record_id),
            reverse=True,
        )
        return hits[:limit]

    def _graph_reachable(self, start: str, max_depth: int) -> list[tuple[str, int]]:
        """Find reachable nodes in graph up to max_depth."""
    ...
    def assess_fact_reliability(self, fact_id: str, *, now: datetime | None = None):
        """Assess reliability of a specific fact."""
    ...
    def reliable_facts(
        self, *, min_confidence: float = 0.0, now: datetime | None = None
    ) -> list[MemoryFact]:
        """Get all facts meeting reliability threshold."""
        return [
            fact
