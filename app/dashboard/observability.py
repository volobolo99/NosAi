...
def _read_evidence(path: Path) -> dict[str, Any]:
    ...
    try:
        ...
        if not isinstance(data, dict):
            return {"status": "FAIL", "error": "evidence is not an object"}
        evidence_commit = data.get("commit")
        current_commit = _current_commit()
        if evidence_commit and current_commit and evidence_commit != current_commit:
            return {"status": "NOT_RUN", "error": "local Test Center evidence is stale for current commit"}
        return data
...
def scan_repository() -> dict[str, Any]:
    ...
    evidence = ci_evidence(); junit = evidence.get("junit", {}); coverage = evidence.get("coverage", {}); ci = evidence.get("ci", {}); security = evidence.get("security", {}); sbom = evidence.get("sbom", {})
    static_ok = ci.get("static") == "success"; security_ok = security.get("status") == "PASS"; sbom_ok = sbom.get("status") == "PASS"
    gates = {"G0": "PASS" if records else "FAIL", "G1": "PASS" if records and parsed == len(records) else "FAIL", "G2": "PASS" if static_ok and security_ok and sbom_ok else ("WARN" if not security else "FAIL"), "G3": junit.get("status", "NOT_RUN"), "G4": ci.get("e2e", "NOT_RUN"), "G5": "FAIL" if unsafe else "PASS", "G6": "PASS" if coverage.get("status") == "PASS" else "WARN"}
