...
class ToyWM:
    def predict(self, s, a):
        d = float(a.parameters.get('delta', 0))
        r = float(a.parameters.get('reward', d))
...
def actions():
    return [Action('safe', {'delta': 1, 'reward': 2}), Action('risky', {'delta': -1, 'reward': 3})]
...
def test_mcts_reuses_expansion_predictions():
    class CountingWM(ToyWM):
        def __init__(self):
