...
class TelemetryOverlayViewWithGesture @JvmOverloads constructor(
    ...
    private val detector = GestureDetector(context, object : GestureDetector.SimpleOnGestureListener() {
        ...
        override fun onFling(e1: MotionEvent?, e2: MotionEvent?, velX: Float, velY: Float): Boolean {
            ...
            val dx = e2.x - e1.x
            val dy = e2.y - e1.y
            if (abs(dx) > abs(dy) && abs(dx) > 100f && abs(velX) > 100f) {
                shiftMode(dx > 0f)
                return true
            }
            return false
    ...
    override fun onTouchEvent(event: MotionEvent): Boolean {
        ...
        if (event.action == MotionEvent.ACTION_UP) {
            findClosestTelemetry(currentVideoPTS)?.let { frame ->
                frame.detectedObjectsList.firstOrNull { box ->
                    event.x in (box.xMin * width)..(box.xMax * width) &&
                        event.y in (box.yMin * height)..(box.yMax * height)
                }?.let { box ->
    ...
    private fun shiftMode(forward: Boolean) {
        ...
        currentMode = modes[(currentMode.ordinal + delta + modes.size) % modes.size]
